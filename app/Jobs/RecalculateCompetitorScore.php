...
class RecalculateCompetitorScore implements ShouldQueue
{
    ...
    protected $ignoreResults = [
        ...
        // Compilation error do not add to penality
    ...
    protected function updateCompetitorScore(Submission $submit, int $computedPontuation, int $penality)
    {
        ...
        } elseif ($score->score < $computedPontuation || ($score->score == $computedPontuation && $score->penality > $penality)) {
            $score->update([
                'score' => $computedPontuation,
                'penality' => $penality,
                'submission_id' => $submit->id,
    ...
    protected function calculateScore(Contest $contest, Submission $submission, $percent = 1): int
    {
        ...
        if ($contest->time_based_points || $contest->parcial_solution) {
            $score *= 1000 - $this->competitor->penality;
        }      // Multiply by 1000
